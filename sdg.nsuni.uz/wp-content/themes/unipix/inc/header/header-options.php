<?php
  if (is_home() && !is_front_page() || is_home() && is_front_page()){
		...
		$skew_style        = (!empty($unipix_option['skew_off'])) ? 'reactheme-skew-head' : '';
...
$main_menu_center = (!empty($unipix_option['main_menu_center'])) || ($menu_center_page == 'yes')  ? 'main-menu-center' : '';

$main_menu_icon = (!empty($unipix_option['main_menu_icon'])) ? 'main-menu-icon-hide' : '';

if ($header_width_meta != ''){
    $header_width = ( $header_width_meta == 'full' ) ? 'container-fluid': 'container';
}else{
    $header_width = !empty($unipix_option['header-grid']) ? $unipix_option['header-grid'] :'';
