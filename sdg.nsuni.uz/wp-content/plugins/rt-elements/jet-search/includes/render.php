...
class Jet_Search_Render {
	...
	public function get_categories_list() {
		$settings           = $this->get_settings_for_display();
		$show_category_list = ! empty( $settings['show_search_category'] ) ? $settings['show_search_category'] : false;
		...
		$select_wrapper_html = apply_filters( 'jet-search/ajax-search/categories-wrapper-select', '<div class="jet-ajax-search__categories">%1$s%2$s%3$s</div>' );
		$select_icon_html    = apply_filters( 'jet-search/ajax-search/categories-select-icon', '
			<span class="jet-ajax-search__categories-select-icon">
				<i aria-hidden="true" class="rt rt-angle-down"></i>
			</span>'
		);
		$select_icon_html2 = apply_filters( 'jet-search/ajax-search/categories-icon','
		...
		return sprintf( $select_wrapper_html, $select_icon_html2, $categories_list, $select_icon_html );
	}
	/**
	 * Get categories button list 
	 *
	 * @since  1.0.0
	 * @since  1.0.1 Added 'id' argument for 'get_terms' function.
	 * @since  1.1.0 Added filter `jet-search/ajax-search/categories-btn/args`
	 * @return string
	 */
	public function get_categories_list2() {
		...
		$taxonomy = !empty($settings['search_taxonomy']) ? $settings['search_taxonomy'] : 'category';
		$include_ids = !empty($settings['category_list']) ? $settings['category_list'] : array();
		$exclude_ids = !empty($settings['exclude_terms_ids']) ? $settings['exclude_terms_ids'] : array();
	
		$args = apply_filters('jet-search/ajax-search/categories-btn/args', array(
			'taxonomy' => $taxonomy,
			'include' => $include_ids,
			'exclude' => $exclude_ids,
			'orderby' => 'name',
		...
		foreach ($categories as $category) {
			$category_link = '<a href="' . get_term_link($category) . '" class="single-tags-banner">' . $category->name . '</a>';
