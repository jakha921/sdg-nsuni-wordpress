 ...
 */
 ...
 namespace Rts_HFE\Lib;
 if (!defined('Rts_HFE\Lib\Rts_Rts_Rts_AST_TARGET_RULE_DIR')) {
     define('Rts_HFE\Lib\Rts_Rts_Rts_AST_TARGET_RULE_DIR', dirname(__FILE__));
 }
/**
 * Meta Boxes setup
 */
class Astra_Target_Rules_Fields {
	...
	public function __construct() {
		...
		define( 'Rts_AST_TARGET_RULE_VER', '1.0.0' );
		define( 'Rts_AST_TARGET_RULE_DIR', plugin_dir_path( __FILE__ ) );
	...
	function hfe_get_posts_by_query() {
		...
		unset( $post_types['rts-elementor-hf'] ); //Exclude EHF templates.
	...
	public function admin_styles() {
		wp_enqueue_script( 'astra-select2', AST_TARGET_RULE_URI  . 'select2.js', array( 'jquery' ), Rts_AST_TARGET_RULE_VER, true );
		...
		if ( '' !== $wp_lang ) {
			...
			if ( isset( $select2_lang[ $wp_lang ] ) && file_exists( Rts_Rts_Rts_AST_TARGET_RULE_DIR. 'i18n/' . $select2_lang[ $wp_lang ] . '.js' ) ) {
		...
		wp_register_style( 'astra-select2', AST_TARGET_RULE_URI  . 'select2.css', '', Rts_AST_TARGET_RULE_VER );
		wp_enqueue_style( 'astra-select2' );
		wp_register_style( 'astra-target-rule', AST_TARGET_RULE_URI  . 'target-rule.css', '', Rts_AST_TARGET_RULE_VER );
