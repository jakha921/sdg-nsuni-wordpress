...
class Rts_HFE_Admin {
	 ...
	 * Instance of Rts_HFE_Admin
	 *
	 * @return Rts_HFE_Admin Instance of Rts_HFE_Admin
	 */
	...
	public static function hfe_admin_enqueue_scripts( $hook ) {
		...
		wp_register_style(
			'hfe-style',
			Rts_HFE_URL . 'assets/css/style.css',
			[],
			Rts_HFE_VER
		);
	...
	private function __construct() {
		...
		add_filter( 'manage_rts-elementor-hf_posts_columns', [ $this, 'set_shortcode_columns' ] );
		add_action( 'manage_rts-elementor-hf_posts_custom_column', [ $this, 'render_shortcode_column' ], 10, 2 );
		...
		if ( is_admin() ) {
			add_action( 'manage_rts-elementor-hf_posts_custom_column', [ $this, 'column_content' ], 10, 2 );
			add_filter( 'manage_rts-elementor-hf_posts_columns', [ $this, 'column_headings' ] );
			require_once Rts_HFE_DIR . 'admin/class-hfe-addons-actions.php';
		}
	}
	/**
	...
	public function register_hfe_epro_script() {
		$ids_array = [
			[
				'id'    => Rts_get_hfe_header_id(),
				'value' => 'Header',
			],
			[
				'id'    => Rts_get_hfe_footer_id(),
				'value' => 'Footer',
			],
			[
				'id'    => Rts_hfe_get_before_footer_id(),
				'value' => 'Before Footer',
			],		
			[
				'id'    => Rts_hfe_get_Rts_topbar_id(),
				'value' => 'Topbar Header',
			],			
			[
				'id'    => Rts_hfe_get_Rts_after__header_id(),
				'value' => 'After Header',
			],
		];

		wp_enqueue_script( 'hfe-elementor-pro-compatibility', Rts_HFE_URL . 'inc/js/hfe-elementor-pro-compatibility.js', [ 'jquery' ], Rts_HFE_VER, true );
	...
	public function header_footer_posttype() {
		$labels = [
			'name'               => __( 'RTS Elementor Header & Footer Builder', 'header-footer-elementor' ),
			'singular_name'      => __( 'RTS Elementor Header & Footer Builder', 'header-footer-elementor' ),
			'menu_name'          => __( 'RTS Elementor Header & Footer Builder', 'header-footer-elementor' ),
			'name_admin_bar'     => __( 'RTS Elementor Header & Footer Builder', 'header-footer-elementor' ),
		...
		register_post_type( 'rts-elementor-hf', $args );
	}
	...
	public function register_admin_menu() {
		...
		// 	__( 'Elementor Header & Footer Builder', 'header-footer-elementor' ),
		// 	'edit_pages',
		// 	'edit.php?post_type=rts-elementor-hf'
		// );

		add_menu_page(
			__( 'RTS Header & Footer Builder', 'textdomain' ),
			'RTS Header & Footer Builder',
			'edit_pages',
			'edit.php?post_type=rts-elementor-hf',
			'',
	...
	function ehf_register_metabox() {
		add_meta_box(
			'ehf-meta-box',
			__( 'RTS Elementor Header & Footer Builder Options', 'header-footer-elementor' ),
			[
				$this,
				'efh_metabox_render',
			],
				...
				<tr class="hfe-options-row type-of-template">
					...
					<td class="hfe-options-row-content">
						<select name="ehf_template_type" id="ehf_template_type">
							...
							<option value="type_Rts_topbar" <?php selected( $template_type, 'type_Rts_topbar' ); ?>><?php _e( 'Topbar', 'header-footer-elementor' ); ?></option>
							
							<option value="Rts_type_header" <?php selected( $template_type, 'Rts_type_header' ); ?>><?php _e( 'Header', 'header-footer-elementor' ); ?></option>
							<option value="type_Rts_after__header" <?php selected( $template_type, 'type_Rts_after__header' ); ?>><?php _e( 'After Header', 'header-footer-elementor' ); ?></option>
							...
							<option value="Rts_type_footer" <?php selected( $template_type, 'Rts_type_footer' ); ?>><?php _e( 'Footer', 'header-footer-elementor' ); ?></option>
	...
	public function location_notice() {
		...
		if ( 'post.php' != $pagenow || ! is_object( $post ) || 'rts-elementor-hf' != $post->post_type ) {
			return;
		}
		...
		if ( '' !== $template_type ) {
			$templates = Rts_Header_Footer_Elementor::get_template_id( $template_type );
	...
	public function block_template_frontend() {
		if ( is_singular( 'rts-elementor-hf' ) && ! current_user_can( 'edit_posts' ) ) {
	...
	function load_canvas_template( $single_template ) {
		...
		if ( 'rts-elementor-hf' == $post->post_type ) {
