...
class RTS_Navigation_Menu extends Widget_Base {
	...
	protected function register_general_content_controls() {
		...
		$this->add_control(
		    'rt_enable_item_animition',
		    [
		        'label' => esc_html__( 'Enable Hover Animation (Note: Use Only Vertical Style)', 'back' ),
		...
		$this->start_controls_tabs( 'tabs_menu_item_style' );
				...
				$this->add_control(
					...
					[
						...
						'selectors' => [
							...
							'{{WRAPPER}} .right_icon_yes ul li a:after' => 'color: {{VALUE}} !important',
						],
					]
				);
				$this->add_control(
					...
					$this->add_control(
						...
						[
							...
							'selectors' => [
								...
								'{{WRAPPER}} .right_icon_yes ul li:hover a:after ' => 'color: {{VALUE}} !important',								
							],
						]
					);		
					$this->add_control(
						'hover_bg_color_main_menu',
						[
							'label'     => __( 'Background Color', 'rtelements' ),
							'type'      => Controls_Manager::COLOR,
							'default'   => '',
							'selectors' => [
								'{{WRAPPER}} .menu-area .navbar ul.menu > li:hover' => 'background-color: {{VALUE}}',
							],
			...
			$this->add_responsive_control(
				...
				[
					...
					'size_units'         => [ 'px', '%' ],
					'selectors'          => [
						'{{WRAPPER}} .menu-area .navbar ul li'  => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}} !important;'
					],
					'frontend_available' => true,
				]
			);
		...
		$this->add_responsive_control(
		    ...
		    [
		        ...
		        'selectors' => [
		            ...
		            '.offcanvas-icon.layout-1 li a svg rect' => 'fill: {{VALUE}} !important;',
		        ],
		    ]
		);
	...
	<div class="menu-area <?php echo esc_attr($settings['layout'].$class_click_expand);?> separator_<?php echo esc_attr( $settings['seperator_on_click']);?> <?php echo esc_attr($class_responsvie_icon); ?>">         
		<div class="menu_one <?php echo esc_attr($settings['parent_icon_style']);?> <?php echo $settings['enable_two_column'];?>">							 
			<div class="col-cell menu-responsive <?php echo esc_attr($class_responsvie);?> right_icon_<?php echo $settings['enable_right_icon'];?> left_icon_<?php echo $settings['enable_left_icon']; ?> enable_anim_<?php echo esc_attr($settings['rt_enable_item_animition']); ?>">  
				<?php
				$menus = $this->get_available_menus();
				if ( empty( $menus ) ) {
					return false;
