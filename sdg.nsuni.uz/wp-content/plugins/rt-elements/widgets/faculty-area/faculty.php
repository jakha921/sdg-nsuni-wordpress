...
class RTS_Faculty_Area_Widget extends \Elementor\Widget_Base {
    ...
    public function get_title() {
        return esc_html__( 'RT Faculty Area', 'rtelements' );
    }
	...
	protected function register_controls() {
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .faculty-pagination-load-more' => 'margin-top: {{SIZE}}{{UNIT}};',
				],
			]
		);
        $this->end_controls_section();
        ...
        $this->add_control(
            ...
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} .faculty.search-filter' => 'border-color: {{VALUE}};',
                ],
            ]
        ); 
        $this->add_control(
			'separator__height',
			[
				'label' => esc_html__( 'Divider Height', 'rtelements' ),
				'type' => \Elementor\Controls_Manager::SLIDER,
				'size_units' => [ 'px', '%', 'em', 'rem', 'custom' ],
				'range' => [
					'px' => [
				...
				'selectors' => [
					'{{WRAPPER}} .faculty.search-filter' => 'border-width: {{SIZE}}{{UNIT}};',
				],
			]
		);
        $this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', '%', 'em', 'rem', 'custom' ],
				'selectors' => [
					'{{WRAPPER}} .faculty.search-filter' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
        $this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', '%', 'em', 'rem', 'custom' ],
				'selectors' => [
					'{{WRAPPER}} .faculty.search-filter' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
        $this->add_control(
            ...
            [
                ...
                'separator' => 'before',
                'selectors' => [
                    '{{WRAPPER}} .category-search h3' => 'color: {{VALUE}};',
                ],
            ]
        ); 
        $this->add_group_control(
            ...
            [
                ...
                'selector' => '{{WRAPPER}} .category-search h3',
	...
	protected function render() {
        ...
        <div class="faculty search-filter">
            <div class="faculty category-search">
                <?php 
                if(!empty($settings['search_label'])) : ?>
                    <h3><?php echo wp_kses_post($settings['search_label']); ?></h3>
                    <?php 
                endif; ?>
        ...
        <div class="all-faculty-category">  
            ...
            <?php 
                ...
                $x=1;
                $cat = $settings['rt-faculty-category'];		      
                ...
                while($best_wp->have_posts()): $best_wp->the_post();					
                    $termsArray  = get_the_terms( $best_wp->ID, "rt-faculty-category" );  //Get the terms for this particular item
        ...
        <?php 
            $post_counts = wp_count_posts('rt-faculty'); 
            $total_posts = isset($post_counts->publish) ? $post_counts->publish : 0;
            $per_page = !empty($settings['per_page']) ? $settings['per_page'] : ''; 
            if ($settings['faculty_pagination'] == 'yes' && $total_posts > $per_page) { 
                if(!empty($settings['faculty_pagination_text'])) : ?>
                    <div class="faculty-pagination-load-more text-center">
                        <button class="load-more-btn load" current-page="2"><?php echo wp_kses_post($settings['faculty_pagination_text']); ?>
                            <span class="ajax-loader"><img src="<?php echo plugin_dir_url(__FILE__) ?>/white-spinner.gif"></span>
                        </button>
				...
				(function ($) {
					$(document).ready(function () {
                        ...
                        $('.search-filter .category-search .cat-search-form input, .search-filter .category-filter select#cat-filter').on('keyup change paste', function (e) { 
                            ...
                            let paginationBtn = $('.faculty-pagination-load-more .load-more-btn');
                            
							$.ajax({
								type: "post",
								url: rtajax.ajaxurl,
								data: {
									action: 'getFacultySearchResult',
								...
								success: function (response) {
									setTimeout(() => {
										if(response){
											...
											$('.all-faculty-category .row').html(response);

											let result_count = $('.all-faculty-category .row .result-col').length;
						...
						$('.faculty-pagination-load-more .load-more-btn').click(function (e) { 
							e.preventDefault();
							$('.faculty-pagination-load-more .ajax-loader').css('display', 'inline-block');
                            $('.full-screen-ajax-loader').css('display', 'inline-block');
							...
							$.ajax({
								...
								success: function (response) {
									setTimeout(() => {
										if(response){
											$('.faculty-pagination-load-more .ajax-loader').css('display', 'none');
											$('.full-screen-ajax-loader').css('display', 'none');
											$('.all-faculty-category .row').append(response);
